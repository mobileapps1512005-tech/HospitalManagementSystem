...
public class AmbulanceUpdateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ambulance_update);

        EdName = findViewById(R.id.EdName);
        EdEmail = findViewById(R.id.EdEmail);
        SpinExperience = findViewById(R.id.SpinExperience);
        EdLicense = findViewById(R.id.EdLicense);
        EdDriverContact = findViewById(R.id.EdDriverContact);
        EdVehicleNumber = findViewById(R.id.EdVehicleNumber);
        EdHospitalName = findViewById(R.id.EdHospitalName);
        EdHospitalContact = findViewById(R.id.EdHospitalContact);
        EdBaseFare = findViewById(R.id.EdBaseFare);
        EdPerKmCost = findViewById(R.id.EdPerKmCost);
        EdEmergencyCost = findViewById(R.id.EdEmergencyCost);
        SpinDoctor = findViewById(R.id.SpinDoctor);
        SpinWithOutDoctor = findViewById(R.id.SpinWithOutDoctor);
        SpinAC = findViewById(R.id.SpinAC);
        SpinWithoutAC = findViewById(R.id.SpinWithoutAC);
        SpinAmbulancePack = findViewById(R.id.SpinAmbulancePack);
        BtnSubmit = findViewById(R.id.BtnSubmit);
        ambulanceDataBase = new AmbulanceDataBase(getApplication());
        ...
        EdLicense.setText(getIntent().getStringExtra("license"));
        EdDriverContact.setText(getIntent().getStringExtra("driver"));
        EdVehicleNumber.setText(getIntent().getStringExtra("vehicle"));
        EdHospitalName.setText(getIntent().getStringExtra("hospitalName"));
        EdHospitalContact.setText(getIntent().getStringExtra("hospital"));
        EdBaseFare.setText(getIntent().getStringExtra("baseFare"));
        EdPerKmCost.setText(getIntent().getStringExtra("perKm"));
        EdEmergencyCost.setText(getIntent().getStringExtra("emergency"));
        WithDoctor = getIntent().getStringExtra("doctor");
        WithoutDoctor = getIntent().getStringExtra("withOutDoctor");
        WithAC = getIntent().getStringExtra("ac");
        WithoutAC = getIntent().getStringExtra("withOutAc");
        AmbulanceAllPackage = getIntent().getStringExtra("All");
    ...
    public void SetInsertAmbulanceDetails(){
        ...
        if (name.isEmpty()){
            ...
            Toast.makeText(getApplication(), "Please Select Valid Experience", Toast.LENGTH_SHORT).show();
            return;
        } if (WithDoctor.equals("Please Select With Doctor Cost")){
            Toast.makeText(getApplication(), "Please Select Valid WithDoctor", Toast.LENGTH_SHORT).show();
            return;
        } if (WithoutDoctor.equals("Please Select Without Doctor Cost")){
            Toast.makeText(getApplication(), "Please Select Valid WithoutDoctor", Toast.LENGTH_SHORT).show();
            return;
        } if (WithAC.equals("please Select With AC Cost")){
            Toast.makeText(getApplication(), "Please Select Valid WithAc", Toast.LENGTH_SHORT).show();
            return;
        } if (WithoutAC.equals("please Select WithOut AC Cost")){
            Toast.makeText(getApplication(), "Please Select Valid WithOutAc", Toast.LENGTH_SHORT).show();
            return;
        } if (AmbulanceAllPackage.equals("Please Select All Ambulance Kit Cost")){
            Toast.makeText(getApplication(), "Please Select Valid AmbulancePackage", Toast.LENGTH_SHORT).show();
            return;
        }
        boolean b = ambulanceDataBase.SetupdateAmbulanceData(id,name,email,Experience,license,DriverContact,VehicleNo,HospitalName,HospitalNo,BaseFare,PerKmCost,EmergencyCost,WithDoctor,WithoutDoctor,WithAC,WithoutAC,AmbulanceAllPackage);
        if (b==true){
    ...
    public void SetAmbulanceWithDoctor(){
        ...
        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<>(getApplication(), android.R.layout.simple_list_item_1,AmbulanceWithDoctor);
        arrayAdapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
        SpinDoctor.setAdapter(arrayAdapter);

        if (AmbulanceWithDoctor!=null){
            int position  = arrayAdapter.getPosition(WithDoctor.trim());
            if (position>0){
                SpinDoctor.setSelection(position);
            }
        }
    }
    ...
    public void SetAmbulanceWithoutDoctor(){
        ...
        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<>(getApplication(), android.R.layout.simple_list_item_1,AmbulanceWithOutDoctor);
        arrayAdapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
        SpinWithOutDoctor.setAdapter(arrayAdapter);
        if (AmbulanceWithOutDoctor!=null){
            int position = arrayAdapter.getPosition(WithoutDoctor.trim());
            if (position>0){
                SpinWithOutDoctor.setSelection(position);
            }
        }
    }
    ...
    public void SetAmbulanceWithAc(){
        ...
        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<>(getApplication(), android.R.layout.simple_list_item_1,AmbulanceWithAc);
        arrayAdapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
        SpinAC.setAdapter(arrayAdapter);

        if (AmbulanceWithAc!=null){
            int position = arrayAdapter.getPosition(WithAC.trim());
            if (position>0){
                SpinAC.setSelection(position);
            }
        }
    ...
    public void SetAmbulanceWithoutAc(){
        ...
        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<>(getApplication(), android.R.layout.simple_list_item_1,AmbulanceWithOutAc);
        arrayAdapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
        SpinWithoutAC.setAdapter(arrayAdapter);

        if (AmbulanceWithOutAc!=null){
            int position = arrayAdapter.getPosition(WithoutAC.trim());
            if (position>0){
                SpinWithoutAC.setSelection(position);
            }
        }
    ...
    public void SetAmbulanceAllPackage(){
        ...
        ArrayAdapter<String> arrayAdapter = new ArrayAdapter<>(getApplication(), android.R.layout.simple_list_item_1,AmbulanceWithAllPackage);
        arrayAdapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
        SpinAmbulancePack.setAdapter(arrayAdapter);

        if (AmbulanceWithAllPackage!=null){
            int position = arrayAdapter.getPosition(AmbulanceAllPackage.trim());
            if (position>0){
                SpinAmbulancePack.setSelection(position);
            }
        }
