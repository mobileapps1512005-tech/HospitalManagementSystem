...
public class AmbulanceApprovalAmbulanceActivity extends AppCompatActivity {
    EditText EdName,EdEmail,EdPhone,EdAlterNumber,EdLocation;
    Spinner SpinAmbulance;
    Button BtnSubmit;
    String name,email,phone,alterNumber,location,ambulances;
    AmbulanceBookingDataBase ambulanceBookingDataBase;
    AmbulanceApprovalDataBase ambulanceApprovalDataBase;
    int id;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ambulance_approval_ambulance);
        ...
        ambulanceApprovalDataBase = new AmbulanceApprovalDataBase(getApplication());
        ...
        EdAlterNumber.setText(getIntent().getStringExtra("alter"));
        EdLocation.setText(getIntent().getStringExtra("location"));
        ambulances = getIntent().getStringExtra("ambulance");


        EdName.setEnabled(false);
        EdEmail.setEnabled(false);
        EdPhone.setEnabled(false);
        EdAlterNumber.setEnabled(false);
        EdLocation.setEnabled(false);
        SpinAmbulance.setEnabled(false);
        ...
        SpinAmbulance.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ambulances = parent.getItemAtPosition(position).toString();
            }
    ...
    public void InsertData(){
        ...
        if (name.isEmpty()){
            ...
            return;
        }if (ambulances.equals("Please Select Ambulance")){
            Toast.makeText(this, "Please Select Valid Option", Toast.LENGTH_SHORT).show();
            return;
        }
        boolean b = ambulanceApprovalDataBase.InsertDataApproval(name,email,phone,alterNumber,location,ambulances);
        if (b==true){
            EdName.setText("");
            EdEmail.setText("");
            EdPhone.setText("");
            ...
            Toast.makeText(this, "Approval SuccessFully....", Toast.LENGTH_SHORT).show();
            ambulanceBookingDataBase.GetDeleteAmbulanceBookDetails(id);
        }else {
    ...
    public void SelectAmbulance(){
        ...
        if (ambulance!=null){
            int position = arrayAdapter.getPosition(ambulances.trim());
            if (position>0){
                SpinAmbulance.setSelection(position);
            }
        }
